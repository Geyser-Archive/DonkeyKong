...
public class CharacterController2D : Raycast
{
    ...
    {
        return this.collisionInfo.Left || this.collisionInfo.Right;
    }
    ...
    private void HorizontalCollisions(ref Vector2 moveAmount)
    {
        ...
        float rayLength = Mathf.Abs(moveAmount.x) + CharacterController2D.WIDTH;

        if (Mathf.Abs(moveAmount.x) < CharacterController2D.WIDTH)
        {
            rayLength = 2f * CharacterController2D.WIDTH;
        }
        ...
        for (int i = 0; i < base.HorizontalRayCount; i++)
        {
            ...
            RaycastHit2D waterHit = Physics2D.Raycast(rayOrigin, Vector2.right * directionX, rayLength, LayerMask.GetMask("Water"));
            ...
            if (hit)
            {
                ...
                if (i == 0 && slopeAngle <= this.maxSlopeAngle)
                {
                    ...
                    {
                        distanceToSlopeStart = hit.distance - CharacterController2D.WIDTH;
                ...
                {
                    moveAmount.x = (hit.distance - CharacterController2D.WIDTH) * directionX;
    ...
    private void VerticalCollisions(ref Vector2 moveAmount)
    {
        ...
        float rayLength = Mathf.Abs(moveAmount.y) + CharacterController2D.WIDTH;
        ...
        for (int i = 0; i < base.VerticalRayCount; i++)
        {
            ...
            if (hit)
            {
                if (hit.collider.CompareTag("OneWay") || hit.collider.CompareTag("Through"))
                {
            ...
            else
            {
                ...
                RaycastHit2D waterHit = Physics2D.Raycast(rayOrigin, Vector2.up * directionY, rayLength, LayerMask.GetMask("Water"));
                ...
                if (waterHit)
                {
                    ...
                    moveAmount.y -= 1f * Time.deltaTime;

                    this.collisionInfo.Below = true;
                    this.collisionInfo.Above = false;
            ...
            if (hit)
            {
                moveAmount.y = (hit.distance - CharacterController2D.WIDTH) * directionY;
        ...
        if (this.collisionInfo.ClimbingSlope)
        {
            ...
            rayLength = Mathf.Abs(moveAmount.x) + CharacterController2D.WIDTH;
    ...
    {
        RaycastHit2D maxSlopeHitLeft = Physics2D.Raycast(base.Origins.BottomLeft, Vector2.down, Mathf.Abs(moveAmount.y) + CharacterController2D.WIDTH, this.collisionLayerMask);
        RaycastHit2D maxSlopeHitRight = Physics2D.Raycast(base.Origins.BottomRight, Vector2.down, Mathf.Abs(moveAmount.y) + CharacterController2D.WIDTH, this.collisionLayerMask);
        ...
        if (!this.collisionInfo.SlidingDownMaxSlope)
        {
            ...
            if (hit)
            {
                ...
                if (slopeAngle != 0f && slopeAngle <= this.maxSlopeAngle)
                {
                    ...
                    {
                        if (hit.distance - CharacterController2D.WIDTH <= Mathf.Tan(slopeAngle * Mathf.Deg2Rad) * Mathf.Abs(moveAmount.x))
                        {
                            float moveDistance = Mathf.Abs(moveAmount.x);

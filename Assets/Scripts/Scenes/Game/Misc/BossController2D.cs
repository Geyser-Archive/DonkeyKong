...
public class BossController2D : Raycast
{
    ...
    public override void Start()
    {
        ...
        this.collisionInfo.FaceDirection = this.faceDirection;
    }
    ...
    private void HorizontalCollisions(ref Vector2 moveAmount)
    {
        ...
        float rayLength = Mathf.Abs(moveAmount.x) + BossController2D.WIDTH;

        if (Mathf.Abs(moveAmount.x) < BossController2D.WIDTH)
        {
            rayLength = 2f * BossController2D.WIDTH;
        }
        ...
        for (int i = 0; i < base.HorizontalRayCount; i++)
        {
            ...
            if (hit)
            {
                ...
                if (i == 0 && slopeAngle <= this.maxSlopeAngle)
                {
                    ...
                    {
                        distanceToSlopeStart = hit.distance - BossController2D.WIDTH;
                ...
                {
                    moveAmount.x = (hit.distance - BossController2D.WIDTH) * directionX;
    ...
    private void VerticalCollisions(ref Vector2 moveAmount)
    {
        ...
        float rayLength = Mathf.Abs(moveAmount.y) + BossController2D.WIDTH;
        ...
        for (int i = 0; i < base.VerticalRayCount; i++)
        {
            ...
            if (hit)
            {
                moveAmount.y = (hit.distance - BossController2D.WIDTH) * directionY;
        ...
        if (this.collisionInfo.ClimbingSlope)
        {
            ...
            rayLength = Mathf.Abs(moveAmount.x) + BossController2D.WIDTH;
    ...
    {
        RaycastHit2D maxSlopeHitLeft = Physics2D.Raycast(base.Origins.BottomLeft, Vector2.down, Mathf.Abs(moveAmount.y) + BossController2D.WIDTH, this.collisionLayerMask);
        RaycastHit2D maxSlopeHitRight = Physics2D.Raycast(base.Origins.BottomRight, Vector2.down, Mathf.Abs(moveAmount.y) + BossController2D.WIDTH, this.collisionLayerMask);
        ...
        if (!this.collisionInfo.SlidingDownMaxSlope)
        {
            ...
            if (hit)
            {
                ...
                if (slopeAngle != 0f && slopeAngle <= this.maxSlopeAngle)
                {
                    ...
                    {
                        if (hit.distance - BossController2D.WIDTH <= Mathf.Tan(slopeAngle * Mathf.Deg2Rad) * Mathf.Abs(moveAmount.x))
                        {
                            float moveDistance = Mathf.Abs(moveAmount.x);

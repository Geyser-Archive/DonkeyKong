...
public class EnemyController2D : Raycast
{
    ...
    public bool WouldFall => this.wouldFall;
    private bool wouldFall;
    ...
    {
        this.collisionLayerMask = 30;
    }

    public bool Left()
    {
        return this.collisionInfo.Left;
    }

    public bool Right()
    {
        return this.collisionInfo.Right;
    }
    ...
    private void HorizontalCollisions(ref Vector2 moveAmount)
    {
        ...
        float rayLength = Mathf.Abs(moveAmount.x) + EnemyController2D.WIDTH;

        if (Mathf.Abs(moveAmount.x) < EnemyController2D.WIDTH)
        {
            rayLength = 2f * EnemyController2D.WIDTH;
        }
        ...
        for (int i = 0; i < base.HorizontalRayCount; i++)
        {
            ...
            if (hit)
            {
                ...
                if (i == 0 && slopeAngle <= this.maxSlopeAngle)
                {
                    ...
                    {
                        distanceToSlopeStart = hit.distance - EnemyController2D.WIDTH;
                ...
                {
                    moveAmount.x = (hit.distance - EnemyController2D.WIDTH) * directionX;
    ...
    private void VerticalCollisions(ref Vector2 moveAmount)
    {
        ...
        float rayLength = Mathf.Abs(moveAmount.y) + EnemyController2D.WIDTH;
        ...
        for (int i = 0; i < base.VerticalRayCount; i++)
        {
            ...
            if (hit)
            {
                moveAmount.y = (hit.distance - EnemyController2D.WIDTH) * directionY;
        ...
        if (this.collisionInfo.ClimbingSlope)
        {
            ...
            rayLength = Mathf.Abs(moveAmount.x) + EnemyController2D.WIDTH;
    ...
    {
        RaycastHit2D maxSlopeHitLeft = Physics2D.Raycast(base.Origins.BottomLeft, Vector2.down, Mathf.Abs(moveAmount.y) + EnemyController2D.WIDTH, this.collisionLayerMask);
        RaycastHit2D maxSlopeHitRight = Physics2D.Raycast(base.Origins.BottomRight, Vector2.down, Mathf.Abs(moveAmount.y) + EnemyController2D.WIDTH, this.collisionLayerMask);
        ...
        if (!this.collisionInfo.SlidingDownMaxSlope)
        {
            ...
            if (hit)
            {
                ...
                if (slopeAngle != 0f && slopeAngle <= this.maxSlopeAngle)
                {
                    ...
                    {
                        if (hit.distance - EnemyController2D.WIDTH <= Mathf.Tan(slopeAngle * Mathf.Deg2Rad) * Mathf.Abs(moveAmount.x))
                        {
                            float moveDistance = Mathf.Abs(moveAmount.x);
    ...
    private void FallCheck()
    {
        ...
        Vector2 origin = new Vector2(base.transform.position.x + (directionX == -1f ? -0.5f : 0.5f), base.transform.position.y);
        RaycastHit2D hit = Physics2D.Raycast(origin, Vector2.down, rayLength, this.collisionLayerMask);
